...
- **Cost guard.** `--max-entries N`, line truncation, and a cheap token
  estimate with a warning before any send.
...
all tests are offline (the HTTP layer is mocked). `python check_logs_ai.py`
must exit 0 on the repo's own log. README test counts are enforced by a
drift-guard CI job, so keep them in sync.
